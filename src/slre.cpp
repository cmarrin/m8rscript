...
static int toi(int x) {
  return m8r::isdigit(x) ? x - '0' : x - 'W';
}

static int hextoi(const unsigned char *s) {
  return (toi(m8r::tolower(s[0])) << 4) | toi(m8r::tolower(s[1]));
}
...
static int match_op(const unsigned char *re, const unsigned char *s,
  ...
  switch (*re) {
    case '\\':
      /* Metacharacters */
      switch (re[1]) {
        case 'S': FAIL_IF(m8r::isspace(*s), SLRE_NO_MATCH); result++; break;
        case 's': FAIL_IF(!m8r::isspace(*s), SLRE_NO_MATCH); result++; break;
        case 'd': FAIL_IF(!m8r::isdigit(*s), SLRE_NO_MATCH); result++; break;
    ...
    default:
      if (info->flags & SLRE_IGNORE_CASE) {
        FAIL_IF(m8r::tolower(*re) != m8r::tolower(*s), SLRE_NO_MATCH);
      } else {
  ...
  while (len <= re_len && re[len] != ']' && result <= 0) {
    ...
    if (re[len] != '-' && re[len + 1] == '-' && re[len + 2] != ']' &&
      ...
      result = info->flags &  SLRE_IGNORE_CASE ?
        m8r::tolower(*s) >= m8r::tolower(re[len]) && m8r::tolower(*s) <= m8r::tolower(re[len + 2]) :
        *s >= re[len] && *s <= re[len + 2];
      len += 3;
    } else {
  ...
  for (i = 0; i < re_len; i += step) {
    ...
    if (re[i] == '|') {
      ...
      if (re[i + 1] == 'x') {
        ...
        FAIL_IF(re[i + 1] ==  'x' && !(m8r::isxdigit(re[i + 2]) &&
                m8r::isxdigit(re[i + 3])), SLRE_INVALID_METACHARACTER);
      } else {
