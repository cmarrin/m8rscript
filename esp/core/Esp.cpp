...
void writeUserData()
{
    ...
    std::shared_ptr<m8r::File> file = m8r::system()->fileSystem()->open(UserDataFilename, m8r::FS::FileOpenMode::Write);
    int32_t count = file->write(reinterpret_cast<const char*>(&_gUserData), sizeof(UserSaveData));
}

void getUserData()
{
    std::shared_ptr<m8r::File> file = m8r::system()->fileSystem()->open(UserDataFilename, m8r::FS::FileOpenMode::Read);
